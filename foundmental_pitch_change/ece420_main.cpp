...
bool lab5PitchShift(float *bufferIn) {
    ...
    if (periodLen > 0) {
        ...
        while(newEpochIdx<2*FRAME_SIZE){
            // find the index of cloest
            int ind = findClosestInVector(epochLocations,newEpochIdx,2,epochLocations.size()-2);

            int start = epochLocations[ind-1];
            int end = epochLocations[ind+1];
            int length = end-start;
            int p = (int)(length/2);
            int windowsize = 2*p+1;
            ind = epochLocations[ind];
            //apply the hanning window
            for (int k = 0; k<windowsize;k++){
                windowed[k] = getHanningCoef(windowsize,k)*(bufferIn[ind-p+k]);
            }
            overlapAddArray(bufferOut,windowed,newEpochIdx-p,windowsize);
            newEpochIdx += F_S/(FREQ_NEW_ANDROID);
        }
